...
class HabilidadeCalculadora(Habilidade):
    ...
    __REGEX_GRUPO_NUMERO_ESQUERDA = 0
    __INDEX_GRUPO_OPERADOR   = 2
    __REGEX_GRUPO_NUMERO_DIREITA = 3
    ...
    def __init__(self, io_manager: IOManager) -> None:
        ...
        self.__avaliador_regex = AvaliadorRegex(HabilidadeCalculadora.__REGEX_ACEITACAO, [
            RegexGrupoInteresse(HabilidadeCalculadora.__REGEX_GRUPO_NUMERO_ESQUERDA, float),
            RegexGrupoInteresse(HabilidadeCalculadora.__INDEX_GRUPO_OPERADOR, str),
            RegexGrupoInteresse(HabilidadeCalculadora.__REGEX_GRUPO_NUMERO_DIREITA, float),
        ])

    def execute_ou_raise(self, comando: str) -> None:
        numero_esquerda, operador, numero_direita = self.__validar(comando)
        r = self.__calcular(numero_esquerda, operador, numero_direita)
        self.__io_manager.imprimir(r)

    def __validar(self, s: str) -> list[Any]:
        try:
    ...
    def __calcular(self, numero_esquerda: float, operador: str, numero_direita: float) -> str:
        match(operador):
            ...
            case '-': return f'{numero_esquerda - numero_direita}'
            case '*': return f'{numero_esquerda * numero_direita}'
            case '/': return self.__t_d(numero_esquerda, numero_direita)
            case _: raise FormatoDesconhecido() # nunca atingido

    def __t_d(self, numero_esquerda: float, numero_direita: float) -> str:
        try:
